...
# System deps (intl, zip, gd, mbstring, soap, xml) + cron + unzip + curl + bash for shopt
RUN apt-get update && apt-get install -y         libicu-dev libzip-dev libpng-dev libjpeg62-turbo-dev libfreetype6-dev         libonig-dev libxml2-dev cron unzip curl git wget bash      && docker-php-ext-configure gd --with-freetype --with-jpeg      && docker-php-ext-install -j$(nproc) gd intl zip mysqli pdo pdo_mysql mbstring xml soap      && a2enmod rewrite headers && rm -rf /var/lib/apt/lists/*

# SuiteCRM version from official releases (see SuiteCRM-Core GitHub releases)
ARG SUITECRM_VERSION=8.8.1
WORKDIR /var/www/html

# Download + unpack official release archive (robust, no rsync)
RUN set -eux;         wget -qO /tmp/suitecrm.zip           "https://github.com/salesagility/SuiteCRM-Core/releases/download/v${SUITECRM_VERSION}/SuiteCRM-${SUITECRM_VERSION}.zip";         unzip -q /tmp/suitecrm.zip -d /tmp;         SUITE_DIR="$(find /tmp -maxdepth 1 -type d -name 'SuiteCRM-*' | head -n1)";         test -n "$SUITE_DIR";         bash -lc 'shopt -s dotglob; cp -a '"$SUITE_DIR"'/* .';         rm -rf /tmp/suitecrm.zip "$SUITE_DIR"
